...
static void build_commit_query(Transaction& txn) {
    ...
    if (strncmp(txn.transaction_type, "DEPOSIT", 7) == 0) {
        ...
        snprintf(txn.commit_query, MAX_QUERY_LEN,
            ...
            txn.amount, txn.transaction_type, now,
            txn.user_id);
        txn.balance_after = txn.user_balance_at_time + txn.amount;

    } else if (strncmp(txn.transaction_type, "TRANSFER", 8) == 0) {
        // TRANSFER: debit sender (atomic guard), then credit recipient ONLY if debit succeeded, then audit.
        // changes() > 0 ensures the next statement only executes if the previous statement updated a row.
        snprintf(txn.commit_query, MAX_QUERY_LEN,
            "UPDATE users SET balance = balance - %.2f "
            ...
            txn.amount, txn.user_id, txn.amount,
            txn.amount, txn.recipient_id,
            txn.transaction_id, txn.user_id,
            txn.amount, txn.transaction_type, now,
            txn.user_id);
        txn.balance_after = txn.user_balance_at_time - txn.amount;
...
// db_update_raw_status uses Tier 1 (global connection + mutex) — no conn needed.
...
void* validator_thread(void* args) {
    ...
    while (true) {
        ...
        if (g_running.load()) usleep(TRANSITION_DELAY_US);

        // ── Session check (Tier 2 — no mutex) ────────────────
        bool session_ok = db_is_session_active(conn, txn.user_id);
        if (!session_ok) {
            std::string log = "Auditing Transaction #" + std::to_string(txn.transaction_id) 
                            + " (" + std::string(txn.transaction_type) + ")\n"
                            + "                                  └─ [FAIL] Session Verification (Not logged in)";
        ...
        if (balance < 0) {
            std::string log = "Auditing Transaction #" + std::to_string(txn.transaction_id) 
                            ...
                            + "                                  └─ [FAIL] Account not found in database";
            logger_log(ThreadType::VALIDATOR, id, log);
        ...
        if ((strncmp(txn.transaction_type, "WITHDRAWAL", 10) == 0 ||
            ...
            std::string log = "Auditing Transaction #" + std::to_string(txn.transaction_id) 
                            + " (" + std::string(txn.transaction_type) + ")\n"
                            + "                                  ├─ [PASS] Session Verification\n"
                            + "                                  └─ [FAIL] Balance Pre-check (Too low)";
            logger_log(ThreadType::VALIDATOR, id, log);
        ...
        std::string log = "Auditing Transaction #" + std::to_string(txn.transaction_id) 
                        + " (" + std::string(txn.transaction_type) + ")\n"
                        + "                                  ├─ [PASS] Session Verification\n"
                        + "                                  ├─ [PASS] Balance Pre-check ($" + std::to_string((int)balance) + " >= $" + std::to_string((int)txn.amount) + ")\n"
                        + "                                  └─ [ACCEPTED] Generated atomic SQL. Forwarded to Updater.";
        logger_log(ThreadType::VALIDATOR, id, log);
    }
